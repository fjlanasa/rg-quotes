...
describe("actions", () => {
  ...
  describe("fetchRateQuotes", () => {
    ...
    it("dispatchs the correct actions on successful fetch request", () => {
      fetchMock.postOnce(REQUEST_RATES_ENDPOINT, {
        body: { requestId: "1" },
        headers: { "content-type": "application/json" }
      });
      fetchMock.getOnce(
        "https://​ss6b2ke2ca.execute-api.us-east-1.amazonaws.com/Prod/ratequotes?requestId=1",
        {
          body: { done: true, rateQuotes: [{ name: "rate1" }] },
          headers: { "content-type": "application/json" }
        }
      );
      ...
      return store
        .dispatch(actions.fetchRateQuotes({ propertyType: "Single" }))
        .then(() => {
