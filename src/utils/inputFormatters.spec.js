...
describe("inputFormatters", () => {
  const { intToCurrencyString, currencyStringToInt, stringToNumber } = inputFormatters;
  describe("intToCurrencyString", () => {
    it("should return empty string if value is null", () => {
      expect(
        intToCurrencyString(null)
      ).toEqual("")
    })

    it("should return string if value is number", () => {
      expect(
        typeof intToCurrencyString(4)
      ).toEqual("string")
    })

    it("should prepend '$' if value is number", () => {
      expect(
        intToCurrencyString(5)
      ).toEqual("$5")
    })

    it("should add ',' as delimiter", () => {
      expect(
        intToCurrencyString(5555)
      ).toEqual("$5,555")

      expect(
        intToCurrencyString(55555555)
      ).toEqual("$55,555,555")
    })
  })
